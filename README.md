...
| `同步每日推荐` | 三平台今日推荐 |
| `同步Boss` / `同步智联` / `同步猎聘` | 单平台同步 |
| `投递汇总` | 全部平台投递统计 |
| `投递表` / `每日推荐表` | 列表展示岗位（最多10条） |
| `导出Excel` | 全部平台一次性导出（6文件：每个平台投递+推荐各一） |
| `导出Boss` / `导出智联` / `导出猎聘` | 单平台导出 |
...
| 指令 | 功能 |
|------|------|
| `查看配置` | 表格化配置中心（模型/Key/Boss/GitHub 状态） |
...
├── boss.py                # Boss直聘 5 模块 + Excel 导出
├── zhaopin.py             # 智联招聘 3 模块 + Excel 导出
├── liepin.py              # 猎聘 5 模块 + Excel 导出
├── skills.py              # 30+ 项技能 + OpenAI Function Calling 定义
├── settings.py            # 模型/Key/Cookie/Token 配置管理
...
├── config.py              # AI 模型配置
├── storage.py             # JSON 持久化
├── chainlit_app.py        # Chainlit UI（Python 3.14 补丁 + 文件上传 + 历史持久化 + 帮助系统）
